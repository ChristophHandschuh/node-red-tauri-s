    ...
    function binary_output(config) {
        ...
        function setPins(pins, state, cmd, signalName="", oldState=""){
            for(let i=0;i<pins.length;i++){
                ...
                node.port.i2cWrite(busAddress, 2, buffer, function(err) {if (err) {
            ...
            //deactivate cmd
            if(cmd){
                node.send([null, {"payload": {"timestamp": Date.now(), "signal": signalName, "state": oldState, "info": "", "modulname": config.modname,"modulnumber": config.modnum}}, null]);
                activeCMD = false;
            }
        }
        ...
        /*** INIT I2C MODULE ***/
        try{
            node.port = i2c.open(parseInt(config.busnum), err => {if (err) throw err;});
            var buffer = new Buffer.from([0x00, 0x00]);
            node.port.i2cWrite(busAddress, 2, buffer, function(err) {if (err) {node.error(err);}});
            buffer = new Buffer.from([0x01, 0x00]);
            node.port.i2cWrite(busAddress, 2, buffer, function(err) {if (err) {node.error(err);}});
            buffer = new Buffer.from([0x12, 0x00]);
            node.port.i2cWrite(busAddress, 2, buffer, function(err) {if (err) {node.error(err);}});
            buffer = new Buffer.from([0x13, 0x00]);
            node.port.i2cWrite(busAddress, 2, buffer, function(err) {if (err) {node.error(err);}});
        } catch(err){
        ...
        node.on('input', function(msg) {
            if(msg.payload === "pinstates"){
                ...
                try{
                    node.port.readWord(busAddress, 0x12, (err, rawData) => {
                        if (err) throw err;

                        pinstates0x12 = rawData;
                        node.port.readWord(busAddress, 0x13, (err, rawData) => {
                            if (err) throw err;
                            pinstates0x13 = rawData; 
                ...
                if(type === "info"){
                    ...
                    msg.payload = {"timestamp": Date.now(), "signal": signalName, "state": oldState, "info": "", "modulname": config.modname,"modulnumber": config.modnum};
                    node.send([null, msg, null]);
                }else if(type === "cmd"){
                    if(activeCMD){
                        msg.payload = {"timestamp": Date.now(), "signal": signalName, "state": oldState, "info": "cmd currently running", "modulname": config.modname,"modulnumber": config.modnum};
                        this.send([null,msg,null]);
                    }else{
                        msg.payload = {"timestamp": Date.now(), "signal": signalName, "state": oldState, "info": "", "modulname": config.modname,"modulnumber": config.modnum};
                        node.send([null, msg, null]);
                ...
                msg.payload = {"timestamp": Date.now(), "signal": "", "state": "", "info": "Could not resole input: "+payload.toString(), "modulname": config.modname,"modulnumber": config.modnum};
                node.send([null, msg, null]);
            }
        });
